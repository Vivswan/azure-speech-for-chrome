...
const copyAssets = async () => {
	...
	const processTailwindCSS = async () => {
		try {
			...
			fs.copyFileSync(path.join(rootDir, "src/assets/css/styles.css"), path.join(distDir, "css/styles.css"));
			...
			if (dir.isDirectory()) {
				...
				if (dir.name === "html") {
					...
					htmlFiles.forEach((htmlFile) => {
						fs.copyFileSync(path.join(srcPath, htmlFile), path.join(distDir, htmlFile));
					});
				} else {
...
const createPackage = async () => {
	...
	const zipPath = path.join(buildsDir, `azure-speech-for-chrome-v${version}.zip`);
	const crxPath = path.join(buildsDir, `azure-speech-for-chrome-v${version}.crx`);
...
async function build() {
	...
	const copyYamlFiles = () => {
		...
		yamlFiles.forEach((file) => {
			fs.copyFileSync(path.join(yamlSourceDir, file), path.join(yamlDestDir, file));
		});
	...
	try {
		...
		if (fs.existsSync(tsAssetsDir)) {
			const tsFiles = fs
				.readdirSync(tsAssetsDir, { recursive: true })
		...
		// Initial build
		const _result = await Bun.build(buildOptions);
		...
		if (isWatchMode) {
			...
			watch(path.join(rootDir, "src/assets"), { recursive: true }, async (eventType, filename) => {
				if (
					filename &&
					!filename.includes("css/styles.css") &&
