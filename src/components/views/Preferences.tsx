	...
	return (
		<div className="flex flex-col gap-5">
			<div className={!sync.credentialsValid ? "opacity-50 pointer-events-none" : ""}>
				<div className="font-semibold text-neutral-700 mb-1.5 ml-1 flex items-center">{t("preferences.title")}</div>
							...
							onChange={(language) => {
								if (languageOptions.find((l: LanguageOption) => l.value === language)) {
									...
									const isCurrentVoiceValid =
										currentVoice && newVoiceOptions.find((v: VoiceOption) => v.value === currentVoice);
...
function getVoiceOptions(session: SessionStorage, language: string, engine: string): VoiceOption[] {
	...
	const voicesInLanguage =
		session.voices?.filter((voice) => voice.locale === language && voice.voiceType === engine) || [];
...
function getEngineOptions(session: SessionStorage, language: string): EngineOption[] {
	...
	const voicesInLanguage = session.voices?.filter((voice) => voice.locale === language) || [];

	const engines = new Set(voicesInLanguage.map((voice) => voice.voiceType));
